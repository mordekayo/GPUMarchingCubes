...
inline Vector3 Vector3::Cross(const Vector3& V0, const Vector3& V1) noexcept
{
	using namespace DirectX;
	XMVECTOR v1 = XMLoadFloat3(&V0);
...
inline float Vector4::Dot3(const Vector4& V) const noexcept
{
	using namespace DirectX;
	XMVECTOR v1 = XMLoadFloat4(this);
	XMVECTOR v2 = XMLoadFloat4(&V);
...
inline Vector3 Vector4::ToVec3() const noexcept
{
...
inline Matrix Matrix::CreateInfinityProj(float fovRad, float aspect) noexcept
{
	using namespace DirectX;

	const float e = 1.0f / tanf(fovRad * 0.5f);
	constexpr float eps = 0.001f;
	Matrix R;
	XMStoreFloat4x4(&R, XMMATRIX(	e/aspect, 0, 0, 0,
									0, e, 0, 0,
									0, 0, eps-1, -1,
									0, 0, -0.001f, 0	));
	return R;
